...
def build_summary_table(
    ...
    rows = [
        ...
        # ── Robustness ─────────────────────────────────────────────────────────
        ("Permutation test p-value", f"{perm['p_value']:.4f}", "< 0.05"),
        (
        ...
        ("CATE std (effect heterogeneity)", f"{cf.cate.std():.4f}", "—"),
        ("% customers CATE < 0 (benefit)", f"{pct_benefit:.1%}", "~95%"),
        ("Confidence score", f"{cf.conf_score:.1%}", "—"),
        ("RATE (targeting quality)", f"{rate_res['rate']:+.4f}", "Positive (TOC)"),
        ("AUQC (QINI curve area)", f"{qini['auqc']:.4f}", "—"),
        # ── Policy ─────────────────────────────────────────────────────────────
        ("Optimal targeting share", f"{policy.best_frac:.1%}", "~95%"),
        (
            "Max profit — smart targeting",
            f"€{policy.max_profit_smart:.2f}",
            "~2× universal",
        ),
        ("Max profit — universal nudging", f"€{policy.max_profit_univ:.2f}", "—"),
        ("Value of personalisation", f"€{policy.personalisation_value:.2f}", "—"),
        (
