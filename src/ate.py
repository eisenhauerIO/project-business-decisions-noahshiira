...
def _ipw(
    Y: np.ndarray, T: np.ndarray, ps: np.ndarray, n_boot: int = 500, seed: int = 42
) -> dict:
    ...
    boots = [
        ...
        for idx in (
            rng.choice(len(Y), size=len(Y), replace=True) for _ in range(n_boot)
        )
    ]
    se = np.std(boots)
...
def _aipw(
    ...
    boots = [
        _dr_ate(y_arr[idx], t_arr[idx], ps[idx], mu1[idx], mu0[idx])
        for idx in (
            rng.choice(idx_all, size=len(idx_all), replace=True) for _ in range(n_boot)
        )
    ]
    se = np.std(boots)
    z = ate / se if se > 0 else np.nan
...
def run_ate_suite(
    ...
    for estimator_fn, args in [
        ...
        try:
            ...
            logger.info(
                "  %-30s ATE=%+.4f  p=%.4f", row["method"], row["ate"], row["pvalue"]
            )
...
def permutation_test(
    ...
    Xmat = sm.add_constant(pd.concat([T, X], axis=1))
    observed_ate = sm.OLS(Y, Xmat).fit(cov_type="HC3").params[T.name]
    ...
    for _ in range(n_permutations):
        ...
        perm_ates.append(sm.OLS(Y, Xmat_p).fit().params[T_perm.name])
...
def bandwidth_sensitivity(
    ...
    for bw in bandwidths:
        ...
        if len(sub) < 50:
            logger.warning(
                "bandwidth_sensitivity: only %d obs at bw=%.2f — skipping", len(sub), bw
            )
            continue
