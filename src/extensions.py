...
def profit_lever_analysis(
    ...
    for rc in rc_list:
        for nc in nc_list:
            ...
            for frac in fracs:
                ...
                p_univ.append(frac * n * max(0.0, -ate_reference) * rc - k * nc)
    ...
    for rc in rc_sweep:
        ...
        pu = [
            f * n * max(0.0, -ate_reference) * rc - int(f * n) * nc_fixed for f in fracs
        ]
    ...
    ax.plot(
        rc_sweep, smart_maxes, color=c["smart"], linewidth=2.5, label="Smart targeting"
    )
    ax.plot(
        rc_sweep,
        univ_maxes,
...
def multi_arm_simulation(cfg: Config) -> dict:
    ...
    cate_env = (
        -0.02 - 0.10 * env_sensitivity * past_return_rate + rng.normal(0, 0.02, n_sim)
    )
    cate_norm = -0.03 - 0.04 * past_return_rate + rng.normal(0, 0.02, n_sim)
    cate_fin = (
        -0.01 - 0.12 * price_sensitivity * past_return_rate + rng.normal(0, 0.02, n_sim)
    )
...
def dynamic_targeting_simulation(cfg: Config) -> pd.DataFrame:
    ...
    for n_obs in ext.sample_sizes:
        ...
        true_cates = rng.normal(
            ext.true_cate_mean, ext.true_cate_std, ext.test_set_size
        )
        est_cates = true_cates + rng.normal(
            0, noise * ext.true_cate_std, ext.test_set_size
        )

        mask_smart = est_cates < 0
        p_smart = (
            -true_cates[mask_smart].clip(max=0)
        ).sum() * rc - mask_smart.sum() * nc
    ...
    if len(crossover):
        print(
            f"Smart targeting becomes profitable at ~{crossover.min():,} training observations."
        )
    print(
        dyn_df[["n_training", "profit_smart", "profit_univ", "personalisation_value"]]
        .round(2)
