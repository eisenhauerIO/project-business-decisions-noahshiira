...
def plot_permutation_null(perm_result: dict) -> plt.Figure:
    ...
    ax.axvline(
        -abs(observed_ate), color="#E07B54", linewidth=2, linestyle="--", alpha=0.6
    )
    ax.set_xlabel("ATE")
...
def plot_cate_distribution(cate_result) -> plt.Figure:
    ...
    axes[0].axvline(
        cate.mean(), color="red", linestyle="--", label=f"Mean CATE = {cate.mean():.4f}"
    )
    ...
    axes[1].set_title(
        f"Share Benefiting from Nudge\n(Confidence score: {cate_result.conf_score:.1%})"
    )
...
def plot_feature_importance(feat_imp: pd.Series, top_n: int = 15) -> plt.Figure:
    ...
    ax.set_title(
        f"Causal Forest — Top {top_n} Feature Importances\n(heterogeneity drivers)"
    )
    fig.tight_layout()
    return fig
...
def plot_policy_curve(
    ...
    ax.plot(
        ...
        linewidth=2.5,
        linestyle="--",
        label="Universal nudging (ATE-based)",
    )
...
def plot_cate_segments(seg_stats: pd.DataFrame) -> plt.Figure:
    ...
    fig, ax = plt.subplots(figsize=(8, 4))
    seg_stats["mean_cate"].plot(kind="bar", ax=ax, color=colors, edgecolor="white")
    ax.axhline(0, color="black", linestyle="--", linewidth=1, alpha=0.6)
...
def plot_toc_curve(rate_result: dict) -> plt.Figure:
    ...
    ax.plot(
        ...
        linewidth=1.5,
        linestyle="--",
        label="Random targeting (ATE baseline)",
    )
